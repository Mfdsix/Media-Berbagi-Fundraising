...
class WakafController extends Controller
{
	...
	{
		$projects = Project::where('type', 'wakaf')
		->paginate(15);
		...
		return view('front.wakaf.index')->with([
			'projects' => $projects,
		]);
	}
	...
	public function process($id, Request $request)
	{
		...
		$request->validate([
			'total_buy' => 'required|min:1'
		]);

		Session::put('total_buy', $request->total_buy);

		return redirect('/wakaf/'.$project->id.'/nominal');
	}

	public function nominal($id)
	{
		$project = Project::findOrFail($id);
		return view('front.wakaf.nominal')->with([
			'project' => $project,
		]);
	}
	...
	public function choose_payment($id)
	{
		...
		return view('front.wakaf.payment')->with([
			'project' => $project,
			'banks' => $banks,
			'payment' => $payment,
	...
	{		
		...
		return redirect('/wakaf/'.$id.'/nominal?n='.$request->n);
	}
