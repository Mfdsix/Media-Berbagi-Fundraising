			...
			<div class="box-body d-flex align-items-center pd-7 pb-0 row">
				...
				<div class="col-md-6 mb-0">
					<div class="btn-now text-end d-block" id="statistics">
						<a class="h6 font-w500 text-end" href="{{ url('/accounting/manual_donation/create') }}"><span>Input Transaksi</span></a>
					</div>
				</div>
			</div>
		</div>
				...
				<table class="table table-striped mt-3">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<a href="{{ url('accounting/manual_donation/'.$v->id.'/edit') }}" class="btn btn-sm btn-warning"><i class="bx bx-edit"></i></a>
								<button data-id="{{ $v->id }}" class="btn btn-sm btn-danger btn-delete-trigger"><i class="bx bx-trash"></i></button>
								<a target="_blank" href="{{ url('accounting/manual_donation/'.$v->id.'/receipt') }}" class="btn btn-sm btn-primary"><i class="bx bx-printer"></i></a>
							</td>
						</tr>
						@endforeach
					</tbody>
...
<script type="text/javascript">
	$(document).ready(function(){
		$(".btn-delete-trigger").on("click", function(){
			var id = $(this).data('id');
			$("#form-delete").attr('action', '{{ url("accounting/manual_donation") }}'+'/'+id);
		...
		@if(request()->receipt)
		window.open("/accounting/manual_donation/" + '{{ request()->receipt }}' + '/receipt');
		@endif
	});
</script>
@endsection
