		...
		<div class="box box-primary">
			<div class="box-body pb-0">
				<div class="btn-now d-block py-0" id="statistics">
					<a class="h6 font-w500" href="{{ url('/gerai/manual_donation') }}"><span>Kembali</span></a>
				</div>
			</div>
		</div>
	</div>
...
<div class="row mt--2">
	<div class="col-md-12">
		<form class="box" enctype="multipart/form-data" method="post" action="{{ isset($data) ? url('gerai/manual_donation/'.$data->id) : url('gerai/manual_donation') }}">
			<div class="bix-body">
				<div class="row">
					@if(isset($data))
					...
					<div class="col-md-12 col-lg-12">
						...
						<div class="mb-4">
							...
							<select name="fundraiser_id" class="form-control selectpicker" data-live-search="true">
								...
								<option @if(isset($data) && $data->referral_id == $v->user_id) selected @endif value="{{ $v->id }}">{{$v->id}} {{ $v->fullname }}</option>
								@endforeach
							</select>
							@error('fundraiser_id')
							<small class="form-text text-danger">{{ $message }}</small>
