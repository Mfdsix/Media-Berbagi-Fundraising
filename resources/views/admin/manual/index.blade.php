			...
			<div class="box-body d-flex align-items-center pd-7 pb-0 row">
				...
				<div class="col-md-6 mb-0">
					<div class="btn-now text-end d-block" id="statistics">
						<a class="h6 font-w500 text-end" href="{{ url('/admin/manual_donation/create') }}"><span>Input Transaksi</span></a>
						<a class="h6 font-w500 text-end" href="{{ url('/admin/manual_donation/export') }}"><span>Export</span></a>
					</div>
				</div>
			</div>
		</div>
				...
				<table class="table table-striped mt-3" id="table">
					...
					<tbody>
						...
						<tr>
							...
							<td>{{ $v->fundraiser ? $v->fundraiser->name : "" }}</td>
							<td>{{ number_format($v->total, 0, ',', '.') }}</td>
							<td>
								<a href="{{ url('admin/manual_donation/'.$v->id.'/edit') }}" class="btn btn-sm btn-warning"><i class="bx bx-edit"></i></a>
								<button data-id="{{ $v->id }}" class="btn btn-sm btn-danger btn-delete-trigger"><i class="bx bx-trash"></i></button>
								{{-- <a target="_blank" href="{{ url('admin/kwitansi/'.$v->id) }}" class="btn btn-sm btn-primary"><i class="bx bx-printer"></i></a> --}}
								<a href="javascript:void(0)" onclick="window.open('{{ url('invoice/'.$v->id) }}','_blank','height=500,width=500,left=100,top=100,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes')" class="btn btn-sm btn-primary"><i class="bx bx-download"></i></a>
								{{-- <a target="_blank" href="{{ url('admin/manual_donation/'.$v->id.'/receipt') }}" class="btn btn-sm btn-primary"><i class="bx bx-printer"></i></a> --}}
							</td>
						</tr>
						@endforeach
					</tbody>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(".btn-delete-trigger").on("click", function(){
			var id = $(this).data('id');
			$("#form-delete").attr('action', '{{ url("admin/manual_donation") }}'+'/'+id);
		...
		@if(request()->receipt)
		window.open("/admin/manual_donation/" + '{{ request()->receipt }}' + '/receipt');
		@endif
	});
</script>
@endsection
