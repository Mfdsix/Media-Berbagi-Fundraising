			...
			<div class="box-body d-flex align-items-center pd-7 pb-0 row">
				<div class="col-md-6 mb-0">
					<div class="me-auto w-55">
						<h5 class="card-title text-white fs-30 font-w500 mt-4">Penyaluran Dana</h5>
				...
				<div class="col-md-6 mb-0">
					<div class="btn-now text-end d-block" id="statistics">
						<a class="h6 font-w500 text-end" href="{{ url('/admin/funding_distribution/create') }}"><span>Salurkan Dana</span></a>
					</div>
				</div>
			</div>
		</div>
				...
				<table class="table table-striped">
					...
					<tbody>
						...
						<tr>
							...
							<td>Untuk {{ $v->receiver.' '.$v->receiver_unit }}</td>
							<td>{{ date('d M Y', strtotime($v->created_at)) }}</td>
							<td>
								<a href="{{ url('admin/funding_distribution/'.$v->id.'/edit') }}" class="btn btn-sm btn-warning"><i class="bx bx-edit"></i></a>
...
<script type="text/javascript">
	$(document).ready(function(){
		$(".btn-delete-trigger").on("click", function(){
			var id = $(this).data('id');
			$("#form-delete").attr('action', '{{ url("admin/funding_distribution") }}'+'/'+id);
