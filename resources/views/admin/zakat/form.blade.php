		...
		<div class="box box-primary">
			<div class="box-body pb-0">
				<div class="btn-now d-block py-0" id="statistics">
					<a class="h6 font-w500" href="{{ url('/admin/zakat') }}"><span>Kembali</span></a>
				</div>
			</div>
		</div>
	</div>
...
<div class="row">
	<div class="col-md-12">
		<form class="box" enctype="multipart/form-data" action="{{ isset($data) ? url('admin/zakat/'.$data->id) : url('admin/zakat') }}" method="post">
			@csrf
			@if(isset($data))
				...
				<div class="row">
					<div class="col-md-12 col-lg-12">
						...
						<div class="mb-4">
							...
							<input type="text" class="form-control" id="button_label" placeholder="Tunaikan Zakat Sekarang" name="button_label" required="" value="{{ isset($data) ? $data->button_label : old('button_label') }}">
							@error('button_label')
							<small class="form-text text-danger">{{ $message }}</small>
							@enderror
						</div>
					...
					<div class="mb-4">
						<label for="fundraiser_reward">Persentase Untuk Reward Fundraiser (dalam %)</label> <a href="javascript:void(0)" title="biaya akan di hitung dari total nominal pendonasi yang di ajak fundraiser" class="bg-secondary bg-opacity-75 rounded-pill"><i class="bx bx-question-mark"></i></a>
						<div class="input-group">
		...
		<form method="POST" id="status-form" action="{{ url('admin/instant-program/zakat/switch' ) }}">
			@csrf
		</form>
	</div>
</div>
