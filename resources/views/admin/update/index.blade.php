			...
			<div class="box-body d-flex align-items-center pd-7 pb-0 row">
				...
				<div class="col-md-6 mb-0">
					<div class="btn-now text-end d-block" id="statistics">
						<a class="h6 font-w500 text-end" href="{{ url('/admin/update/create') }}"><span>Buat Update</span></a>
					</div>
				</div>
			</div>
		</div>
				...
				<table class="table table-striped mt-3">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<a target="_blank" href="{{ url($v->project->slug.'?kabarterbaru=true') }}" class="btn btn-sm btn-primary">preview</a>
								<a class="btn btn-sm btn-warning" href="{{ url('/admin/update/'.$v->id.'/edit') }}"><i class="bx bx-edit"></i></a>
...
<script type="text/javascript">
	$(document).ready(function(){
		$(".btn-delete-trigger").on("click", function(){
			var id = $(this).data('id');
			$("#form-delete").attr('action', '{{ url("admin/update") }}'+'/'+id);
